...
window.changeMood = function(moodKey) {
    applyDNAMood(moodKey);
};

// --- 2. MOOD LOGIC (Colors + DNA Voice) ---
function applyDNAMood(moodKey) {
    const moodPalette = {
        'happy': { bg: '#fefce8', accent: '#eab308', sidebar: '#854d0e', voice: "DNA Scanner online. Energy levels optimal for creative cooking." },
        'sad':   { bg: '#eff6ff', accent: '#3b82f6', sidebar: '#1e3a8a', voice: "Scanning for comfort-focused genetic markers. Let's find something soothing." },
        'lazy':  { bg: '#fdf2f8', accent: '#ec4899', sidebar: '#831843', voice: "Efficiency mode activated. Searching for quick-synthesis molecular meals." },
        'angry': { bg: '#fef2f2', accent: '#ef4444', sidebar: '#7f1d1d', voice: "High-intensity metabolic search initiated. Finding powerful flavor profiles." },
        'reset': { bg: '#f1f5f9', accent: '#a855f7', sidebar: '#4840a1', voice: "DNA Lab reset to default diagnostic baseline." }
    };
    ...
    document.documentElement.style.setProperty('--accent', theme.accent); // Using your existing --accent variable
...
function speak(text) {
    if ('speechSynthesis' in window) {
        ...
        utterance.rate = 0.95; 
        utterance.pitch = 1.2; // Slightly higher for a "High-Tech Lab" feel
...
async function runDNALab() {
    ...
    speak("Analyzing genetic markers for matching recipes.");
    ...
    try {
        ...
        if (!data.meals) {
            ...
            speak("Scanning complete. Zero genetic matches detected.");
            return;
        }
...
function renderMatches(meals, fridgeItems) {
    ...
    meals.forEach(meal => {
        ...
        grid.innerHTML += `
            <div class="recipe-card" onclick="openModal('${meal.idMeal}')" style="border-top: 4px solid var(--accent);">
                <div class="match-badge" style="background: var(--accent);">${score}%</div>
...
async function openModal(id) {
    ...
    speak(`Opening molecular sequence for ${meal.strMeal}.`);
    ...
    document.getElementById('modal-body').innerHTML = `
        <h2 style="font-size:2.5rem; margin-bottom:15px; color:var(--accent);">${meal.strMeal}</h2>
