...
function changeMood(mood, showNotify = true) {
    ...
    const statusBox = document.getElementById('mood-status'); // This might be missing on baby.html
    const config = {
        'happy': { bg: '#fefce8', accent: '#eab308', sidebar: '#854d0e', label: 'Happy', voice: "Feeling Happy! Let's cook something bright." },
        'sad':   { bg: '#eff6ff', accent: '#3b82f6', sidebar: '#1e3a8a', label: 'Sad', voice: "It's okay to feel down. Let's make something comforting." },
        'lazy':  { bg: '#fdf2f8', accent: '#ec4899', sidebar: '#831843', label: 'Lazy', voice: "Low effort, high reward. I got you." },
        'angry': { bg: '#fef2f2', accent: '#ef4444', sidebar: '#7f1d1d', label: 'Angry', voice: "Take a deep breath. Let's focus on a great meal." },
        'reset': { bg: '#f1f5f9', accent: '#a855f7', sidebar: '#4840a1', label: 'Default', voice: "System reset to default." }
    };
    ...
    // VOICE LOGIC - Moved outside the statusBox check so it always works!
    if (showNotify) {
        speakMood(choice.voice);
    }
